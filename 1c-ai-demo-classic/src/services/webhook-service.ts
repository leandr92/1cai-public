...
export class WebhookService extends EventEmitter {
  ...
  private async deliverEvent(event: WebhookEvent, endpoint: WebhookEndpoint): Promise<WebhookResponse> {
    ...
    const signature = endpoint.secret ? this.generateSignature(payload, endpoint.secret) : undefined;
  ...
  private updateMetrics(delivery: WebhookDelivery, success: boolean): void {
    ...
    this.metrics.errorRate = (this.metrics.failedEvents / this.metrics.deliveredEvents) * 100;
