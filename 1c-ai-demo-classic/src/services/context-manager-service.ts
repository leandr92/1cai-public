...
export class ContextManagerService {
  ...
  private globalContext = new Map<string, GlobalContextData>();
   ...
   */
  createConversation(userId: string, agentType: ConversationContext['agentType']): string {
  ...
  generateContextSummary(conversationId: string): string {
    ...
    const tasksCompleted = context.sessionData.taskHistory?.filter(t => t.status === 'completed').length || 0;
    const tasksActive = context.sessionData.taskHistory?.filter(t => t.status === 'active').length || 0;
  ...
  importContext(jsonData: string): string {
    try {
      ...
      if (this.conversations.has(contextData.id)) {
        contextData.id = uuidv4(); // Генерируем новый ID
      }
