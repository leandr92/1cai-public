...
export class ProductionReadinessService extends EventEmitter {
        ...
        if (check) {
          ...
          criticalItems.forEach(item => {
            issues.push({
              ...
              severity: score.score < 30 ? 'critical' : score.score < 60 ? 'high' : 'moderate',
  ...
  private generateReadinessRecommendations(
    ...
    lowScores.forEach(score => {
      recommendations.push({
        ...
        priority: score.score < 60 ? 'high' : 'moderate',
        category: score.category,
  ...
  private assessRiskLevel(issues: ReadinessIssue[], blockers: ReadinessBlocker[]): 'low' | 'moderate' | 'high' | 'critical' {
  ...
  private estimateEffort(status: string): 'low' | 'moderate' | 'high' {
  ...
  private estimateEffortHours(effort: 'low' | 'moderate' | 'high'): number {
    switch (effort) {
