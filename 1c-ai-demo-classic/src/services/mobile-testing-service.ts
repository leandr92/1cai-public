...
export interface NetworkConfiguration {
  ...
  speed: 'slow' | 'moderate' | 'fast' | 'ultra-fast';
  latency: number;
  bandwidth: number;
  unstable: boolean;
...
export interface UserPreferenceData {
  ...
  language: string;
  fontSize: 'small' | 'moderate' | 'large' | 'extra-large';
  ...
  private getExtendedDeviceList(): MobileDevice[] {
    return [
      ...this.getPriorityDeviceList(),
      {
        ...
        network: { type: '4g', speed: 'moderate', latency: 50, bandwidth: 100, unstable: false, signal: 'fair' },
        hardware: {
          cpu: { cores: 8, architecture: 'arm64', frequency: 2400, performance: 'moderate' },
          memory: { ram: 4096, available: 2048, performance: 'moderate' },
      ...
      data: {
        input: {
          user: {
            ...
            preferences: { theme: 'auto', language: 'ru', fontSize: 'moderate', accessibility: { highContrast: false, voiceOver: false, reducedMotion: false, largeText: false, closedCaptions: false } },
  ...
  private async runSingleMobileTest(
    ...
    const execution: MobileTestExecution = {
      ...
      configuration: {
        ...
        location: { enabled: true, accuracy: 'moderate', coordinates: { x: 55.7558, y: 37.6176, width: 0, height: 0 } },
  ...
  private createErrorExecution(suiteId: string, scenarioId: string, device: MobileDevice, browser: MobileBrowser, error: Error): MobileTestExecution {
    return {
      ...
      configuration: {
        ...
        location: { enabled: false, accuracy: 'moderate', coordinates: { x: 0, y: 0, width: 0, height: 0 } },
