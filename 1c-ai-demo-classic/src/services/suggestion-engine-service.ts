...
export class SuggestionEngineService {
    ...
    for (const cond of conditions) {
      ...
      switch (operator) {
        ...
        case 'contains':
          ...
          break;
        case 'in':
          if (!contextValue.split(',').includes(value)) return false;
          break;
        default:
          return false;
      }
  ...
  private generateContextualSuggestions(conversationId: string, context: AnalysisContext): Suggestion[] {
    ...
    if (conversation.sessionData.taskHistory && conversation.sessionData.taskHistory.length > 0) {
      suggestions.push(...this.generateTaskBasedSuggestions(context, conversation.sessionData.taskHistory));
    }
