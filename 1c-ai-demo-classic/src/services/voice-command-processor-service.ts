...
export interface VoiceCommandRule {
  ...
  command: string;
  agent?: string;
  parameters: Array<{ name: string; type: 'text' | 'string' | 'number' | 'boolean'; required: boolean }>;
  priority: number;
  enabled: boolean;
  description?: string;
  examples?: string[];
...
export class VoiceCommandProcessorService {
  ...
  private initializeDefaultRules(): void {
    ...
    this.addRule({
      ...
      agent: 'architect',
      parameters: [
        { name: 'patternName', type: 'text', required: true }
      ],
      priority: 10,
      enabled: true,
    ...
    this.addRule({
      ...
      agent: 'developer',
      parameters: [
        { name: 'moduleName', type: 'text', required: true },
        { name: 'moduleType', type: 'text', required: false }
      ],
      priority: 10,
      enabled: true,
    ...
    this.addRule({
      ...
      agent: 'data_analyst',
      parameters: [
        { name: 'dashboardName', type: 'text', required: true }
      ],
      priority: 9,
      enabled: true,
