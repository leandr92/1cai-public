    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-6">
        ...
        {report.alerts && report.alerts.length > 0 && (
          <div className="mb-4">
            <div className="text-sm text-gray-500 mb-2">Алерты ({report.alerts.length}):</div>
    ...
    const mockKPIs = [
      {
        ...{
          ...
          unit: '%',
          calculation: { type: 'percentage' as const, formula: '(actual / planned) * 100' },
          target: 100,
      ...
      },
      {
        ...{
          ...
          unit: '%',
          calculation: { type: 'percentage' as const, formula: '(actual - planned) / planned * 100' },
          target: 0,
          thresholds: { excellent: -2, good: 5, warning: 10, critical: 20 },
          frequency: 'daily' as const,
          dataSource: 'financial_tracking',
          isActive: true,
      ...
      },
      {
        ...{
          ...
          unit: 'балл',
          calculation: { type: 'average' as const, formula: 'sum(quality_scores) / count' },
          target: 4.5,
          thresholds: { excellent: 4.8, good: 4.5, warning: 4.0, critical: 3.5 },
          frequency: 'daily' as const,
          dataSource: 'quality_assessment',
          isActive: true,
          owner: 'qa_specialist'
        },
        currentValue: 4.3,
      ...
      },
      {
        ...{
          ...
          unit: '%',
          calculation: { type: 'average' as const, formula: 'completed_tasks / total_tasks * 100' },
          target: 85,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredReports.map(report => (
            <ReportCard
              ...
              onExport={(id) => onExportReport?.(id, 'pdf')}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredTemplates.map(template => (
            <TemplateCard
              key={template.id}
              template={template}
              onGenerate={(id) => onGenerateReport?.(id, { projectId })}
