...
const DeviceOptimizationView: React.FC<DeviceOptimizationViewProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // touchStatsSubscription.unsubscribe(); // touchStatsSubscription is commented out
  ...
  const updatePreference = (key: keyof MobilePreferences, value: any) => {
    if (preferences) {
      const updatedPreferences = { ...preferences, [key]: value } as MobilePreferences;
    ...
    <div className="device-optimization space-y-6">
      ...
      {deviceInfo && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <DeviceCard title="Касания" icon={<Touchpad />}>
            <div className="space-y-2">
              <MetricCard 
                title="Touch устройство" 
                value={deviceInfo.isTouchDevice ? 'Да' : 'Нет'}
                icon={<Touchpad className="h-4 w-4" />}
