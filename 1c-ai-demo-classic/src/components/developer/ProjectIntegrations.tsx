...
const ProjectIntegrations: React.FC<ProjectIntegrationProps> = ({
  ...
  const getSystemStatus = (systemId: string): SystemStatus | null => {
    return metrics?.systemsStatus.find(s => s.systemId === systemId) || null;
  };
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              {status && (
                <>
                  <span>Успешность: {status.syncSuccessRate}%</span>
                  <span>Ошибки: {status.errorCount}</span>
                  <span>Предупреждения: {status.warningCount}</span>
                </>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span>Интервал:</span>
                    <span>{system.settings.syncInterval} мин</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Webhooks:</span>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    <span>Название:</span>
                    <span className="font-mono">{system.settings.fieldMapping.summary}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Описание:</span>
                    <span className="font-mono">{system.settings.fieldMapping.description}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Исполнитель:</span>
                    <span className="font-mono">{system.settings.fieldMapping.assignee}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Статус:</span>
                    <span className="font-mono">{system.settings.fieldMapping.status}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  const renderOverview = () => {
    ...
    const successRate = metrics?.systemsStatus && metrics.systemsStatus.length > 0 
      ? (metrics.systemsStatus?.reduce((sum, s) => sum + s.syncSuccessRate, 0) || 0) / (metrics.systemsStatus?.length || 1)
      : 0;
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-900">
                  {metrics?.lastSyncDate ? metrics.lastSyncDate.toLocaleDateString('ru-RU') : 'Не было'}
                </div>
                <div className="text-sm text-gray-500">Последняя синхронизация</div>
              </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          <div className="p-6">
            {metrics?.systemsStatus.map(status => (
              <div key={status.systemId} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${
                  ...
                  <div>
                    <div className="font-medium">{status.systemName}</div>
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          <div className="p-6">
            {metrics.systemsStatus?.map(system => (
              <div key={system.systemId} className="flex items-center justify-between py-4 border-b border-gray-100 last:border-b-0">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${
                ...
                <div className="flex items-center space-x-6 text-sm">
                  <div className="text-center">
                    <div className="font-medium">{system.syncSuccessRate}%</div>
                    <div className="text-gray-500">Успешность</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="font-medium">{system.errorCount}</div>
                    <div className="text-gray-500">Ошибки</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="font-medium">{system.warningCount}</div>
                    <div className="text-gray-500">Предупреждения</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="font-medium">
                      {system.lastSyncDate ? system.lastSyncDate.toLocaleDateString('ru-RU') : 'Никогда'}
                    </div>
