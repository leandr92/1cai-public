...
| **Security (DevSecOps)** | Shift-left (SAST/DAST), dependency & secret scanning, policy-as-code | Bandit, Safety, run_full_audit; workflows `secret-scan.yml` (Gitleaks) и `trufflehog.yml` (Trufflehog); Conftest Rego (`policy/kubernetes/*.rego`, `policy/terraform/*.rego`), Semgrep (`security/semgrep.yml`), Checkov/Trivy (`scripts/security/run_checkov.sh`), make `policy-check`, `preflight`; Vault best practices (`docs/ops/vault.md`, CSI пример, Vault Agent, AWS/Azure sync, Terraform `azure-keyvault`, `rotate_vault_secret.sh`, `test_vault_sync.sh`) | Расширить конституцию пунктами по least privilege; рассмотреть OPA/policy-as-code для Terraform/GitOps |
